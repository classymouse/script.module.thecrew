...
#OH - checked
    def worker(self):
        try:
            ...
            self.meta = []
            total = len(self.list)
            c.log(f'[CM DEBUG in lists.py @ 575] total={total}')
            ...
            for r in range(0, total, 50):
                ...
                for i in range(r, r+50):
                    name = self.list[i].get('name')
                    imdb = self.list[i].get('imdb')
                    c.log(f'[CM DEBUG in lists.py @ 576] title={name} wwith imdb {imdb}')
            ...
            c.log(f'[CM DEBUG in lists.py @ 601] Exception raised. Error = {e}', 1)
            pass
    ...
    def movie_info(self, i):
        try:
            ...
            c.log(f'[CM DEBUG in lists.py @ 617] item={item}')
            ...
            c.log(f'[CM DEBUG in lists.py @ 721] error={e}', 1)
            pass
