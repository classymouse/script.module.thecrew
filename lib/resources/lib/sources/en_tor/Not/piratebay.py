...
class source:
    ...
    def base_link(self):
        if not self._base_link:
            self._base_link = cache.get(self.__get_base_url, 0, 'https://%s' % self.domains[0]) #cm - was 120
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for entry in rows:
                try:
                    try:
                        name = re.findall('<td class="vertTh">.+?</td>.+?<td><a href=".+?" title=".+?">(.+?)</a>', entry, re.DOTALL)[0]
                    ...
                    #leechers
                    try:
                        rightList = re.findall('<td align="right">(.+?)</td>', entry, re.DOTALL)
                        r_size = rightList[0]
                        r_seeders = int(rightList[1])
                    except Exception as e:
                        failure = traceback.format_exc()
                        c.log('TPB Exception raised. Traceback:: ' + str(failure))
                        c.log('TPB Exception raised. Error = ' + str(e))
                        continue

                    if self.min_seeders > r_seeders:
                        continue
                    ...
                    try:
                        div = 1 if r_size.endswith(('GB', 'GiB')) else 1024
                        size = float(re.sub('[^0-9|/.|/,]', '', r_size)) / div
                        size = '%.2f GB' % size
                        info.append(size)
                    except Exception:
                        pass
