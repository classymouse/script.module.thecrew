...
class sources:
    ...
    def getSources(self, title, year, imdb, tmdb, season, episode, tvshowtitle, premiered, quality='HD', timeout=30):
            ...
            try:
                ...
                if len(self.sources) > 0:
                    ...
                    if debrid_status:
                        debrid_source_counts = {
                            '4K': sum([len([e for e in self.sources if e['quality'].upper() == '4K' and d.valid_url(e['url'], e['source'])]) for d in debrid_list]),
                            '1080p': sum([len([e for e in self.sources if e['quality'] in ['1440p','1080p'] and d.valid_url(e['url'], e['source'])]) for d in debrid_list]),
                            '720p': sum([len([e for e in self.sources if e['quality'] in ['720p','HD'] and d.valid_url(e['url'], e['source'])]) for d in debrid_list]),
                            'SD': sum([len([e for e in self.sources if e['quality'].upper() == 'SD' and d.valid_url(e['url'], e['source'])]) for d in debrid_list]),
                        }
                ...
                if (i / 2) < timeout:
                    try:
                        ...
                        if self.dev_mode:
                            string_dev= '[COLOR lime]Devmode - Elapsed: %s secs[/COLOR]' % str(int(time.time() - start_time))
                        ...
                        c.log('Exception Raised: %s' % str(e), 1)
                else:
                    try:
        ...
        if self.dev_mode:
            end_time = time.time()
            c.log(f"[CM Debug @ 715 in sources.py] Elapsed: %s secs" % str(int(end_time - start_time)))
            c.log(f"[CM Debug @ 716 in sources.py] len(self.sources) = {len(self.sources)}")
            c.log(f"[CM Debug @ 717 in sources.py] self.sources = {self.sources}")
    ...
    def sourcesProcessTorrents(self, torrent_sources):
        ...
        debrid_services = ['Real-Debrid', 'AllDebrid', 'Premiumize.me']
